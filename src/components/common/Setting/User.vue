...
import { NButton, NDataTable, NInputNumber, NModal, NSelect, NTag, useDialog, useMessage } from 'naive-ui'
import { Status, UserBalanceInfo, UserInfo, UserRole, userRoleOptions } from './model'
import { fetchGetUsers, fetchUpdateUserBalance, fetchUpdateUserRole, fetchUpdateUserStatus } from '@/api'
...
const userBalance = ref(new UserBalanceInfo())
const users = ref([])
const columns = [
  {
    title: 'Email',
    ...
    render(row: any) {
      ...
      actions.push(h(
        NButton,
        {
          ...
          onClick: () => handleEditBalance(row._id, row.balance),
        },
        { default: () => t('chat.updateBalance') },
      ))
...
async function handleUpdateUserBalance() {
  if (!userBalance.value._id) {
    ms.error('User Error')
    return
  }
  if (!userBalance.value.balance) {
    ms.error('User Error')
    return
  }
  // eslint-disable-next-line no-console
  console.log(userBalance.value.balance)
  handleSaving.value = true
  try {
    await fetchUpdateUserBalance(userBalance.value._id, userBalance.value.balance)
    await handleGetUsers(pagination.page)
...
function handleEditBalance(userId: string, balance: number) {
  userBalance.value._id = userId
...
<template>
  ...
  <NModal v-model:show="balanceShow" :auto-focus="false" preset="card" style="width:50%;">
    <div class="p-4 space-y-5 min-h-[200px]">
      <div class="space-y-6">
        <div class="flex items-center space-x-4">
          <span class="flex-shrink-0 w-[100px]">{{ $t('setting.userBalance') }}</span>
          <div class="w-[200px]">
            <NInputNumber v-model:value="userBalance.balance" :min="-1" :max="9999" :step="1" />
          </div>
        </div>
        <div class="flex items-center space-x-4">
          <span class="flex-shrink-0 w-[100px]" />
          <NButton type="primary" :loading="handleSaving" @click="handleUpdateUserBalance()">
            {{ $t('common.save') }}
          </NButton>
        </div>
      </div>
