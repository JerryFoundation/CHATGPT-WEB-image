...
import { fetchClearAllChat, queryUserInfo } from '@/api'

const userBalance = ref('0')

getUserInfo().then((message) => {
  userBalance.value = message || '0'
}).catch((error) => {
  console.error(error)
})
...
function getUserInfo(this: any) {
  return queryUserInfo<{ status: string; message: string; data: number }>()
  ...
  <div class="p-4 space-y-5 min-h-[200px]">
    <div class="space-y-6">
      <div class="flex items-center space-x-4">
        ...
        <div class="w-[200px]">
          <NInput v-model:value="userBalance" :readonly="true" placeholder="" />
        </div>
      </div>
      <div class="flex items-center space-x-4">
