...
router.post('/chat-process', [auth, limiter], async (req, res) => {
  ...
  try {
    ...
    if (user.balance <= 0) {
      res.send({ status: 'Fail', message: '额度不足，请联系管理员充值，qq:2425428393', data: null })
      return
    }
    ...
    try {
      ...
      const userId = req.headers.userId.toString()
      const user = await getUserById(userId)
      if (user.config.chatModel === 'mid-journey')
        await updateUserLimit(userId, user.balance - 2)
      else
        await updateUserLimit(userId, user.balance - 1)
    }
    catch (error) {
...
router.post('/query-user', auth, async (req, res) => {
  try {
    const userId = await getUserId(req)
    ...
    if (user == null || user.status !== Status.Normal)
      throw new Error('用户不存在 | User does not exist.')
    res.send({ status: 'Success', message: user.balance })
  }
  catch (error) {
    res.send({ status: 'Fail', message: error.message, data: null })
  }
...
router.post('/user-balance', rootAuth, async (req, res) => {
  try {
    const { userId, balance } = req.body as { userId: string; balance: number }
    const nowUserId = await getUserId(req)
    const user = await getUserById(nowUserId)
    const isAdmin = user.roles ? user.roles.includes(UserRole.Admin) : false
    if (!isAdmin)
      return
    await updateUserLimit(userId, balance)
    res.send({ status: 'Success', message: '更新成功 | Update successfully' })
  }
  catch (error) {
    res.send({ status: 'Fail', message: error.message, data: null })
